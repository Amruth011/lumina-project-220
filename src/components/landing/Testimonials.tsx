...
export const Testimonials = () => {
  return (
    <section className="bg-[#0D1520] py-32 overflow-hidden">
      <div className="max-w-7xl mx-auto px-6 mb-20 flex flex-col md:flex-row items-end justify-between gap-8">
        <div className="space-y-4">
          ...
          <h2 className="text-5xl md:text-7xl font-serif font-bold text-white">The <span className="italic text-lumina-teal">0.1% Speaks.</span></h2>
        </div>
      ...
      <div className="flex gap-8 px-6 md:px-[calc((100vw-1280px)/2)] overflow-x-auto no-scrollbar pb-12 cursor-grab active:cursor-grabbing">
        {testimonials.map((t, i) => (
          <motion.div
            ...
            className="flex-shrink-0 w-80 md:w-[450px] bg-white/5 backdrop-blur-xl border border-white/10 rounded-[2.5rem] p-10 md:p-12 space-y-8 relative group transition-all hover:border-lumina-teal/30"
          >
            <div className="text-lumina-teal text-6xl font-serif opacity-20 absolute top-8 left-8 select-none">“</div>
            <p className="text-xl md:text-2xl font-serif italic text-white/90 leading-relaxed relative z-10">
              {t.quote}
            </p>
            <div className="pt-4 border-t border-white/10 flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-white/40 text-[10px] uppercase font-display tracking-widest">{t.role} @ {t.company}</p>
              </div>
            </div>
          </motion.div>
        ))}
