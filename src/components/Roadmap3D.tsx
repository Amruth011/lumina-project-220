...
const StepCard = ({ 
  ...
  const rotateY = useTransform(scrollYProgress,
    [Math.max(0, stepStart - 0.1), stepStart],
    [step.align === 'left' ? 30 : -30, 0]
  );

  const markerBorderColor = useTransform(scrollYProgress, [stepStart - 0.05, stepStart], ["rgba(0,0,0,0.1)", "#10B981"]);
  const markerBgColor = useTransform(scrollYProgress, [stepStart - 0.05, stepStart], ["#FFFFFF", "#10B981"]);
  const markerScale = useTransform(scrollYProgress, [stepStart - 0.05, stepStart], [1, 1.5]);
  const iconColor = useTransform(scrollYProgress, [stepStart - 0.05, stepStart], ["#9CA3AF", "#FFFFFF"]);
    ...
    <div className="relative w-full flex items-center">
      ...
      <div className={`w-1/2 ${step.align === 'left' ? 'pr-12 text-right' : 'pl-12 ml-auto text-left'}`}>
        <motion.div 
          ...
          className="p-8 rounded-[2.5rem] bg-card border border-border/50 relative overflow-hidden group hover:border-accent-emerald/30 transition-all duration-500 transform-gpu shadow-soft hover:shadow-[0_20px_40px_rgba(16,185,129,0.08)]"
        >
          ...
          <div className={`flex flex-col ${step.align === 'left' ? 'items-end' : 'items-start'} relative z-10`}>
            <div className="w-16 h-16 rounded-2xl bg-accent-emerald/10 text-accent-emerald flex items-center justify-center mb-6 shadow-sm border border-accent-emerald/20 group-hover:scale-110 transition-transform duration-500">
              <step.icon size={32} />
            </div>
            ...
            <h3 className="text-3xl font-serif text-foreground mb-3 tracking-tight">{step.title}</h3>
...
export const Roadmap3D = () => {
  ...
  // Use spring physics to eliminate jitter and lag
  ...
  const glowPosition = useTransform(smoothProgress, [0, 1], ["0%", "100%"]);
  const emojiScale = useTransform(smoothProgress, [0, 0.5, 1], [1, 1.1, 1]);

  return (
    <div ref={containerRef} className="relative py-32 bg-background overflow-hidden perspective-1000">
      
      {/* Background Ambient Glows */}
      <div className="absolute inset-0 pointer-events-none opacity-[0.03]">
        <div className="absolute top-1/4 left-1/4 w-[600px] h-[600px] bg-accent-emerald rounded-full blur-[100px]" />
        <div className="absolute bottom-1/4 right-1/4 w-[500px] h-[500px] bg-accent-emerald rounded-full blur-[100px]" />
      </div>
      ...
      <div className="max-w-6xl mx-auto px-6 relative z-10">
        <div className="text-center mb-40">
          ...
          >
            <div className="inline-flex items-center justify-center px-4 py-2 rounded-full bg-white border border-border text-foreground shadow-sm text-sm font-bold uppercase tracking-[0.2em] mb-8">
              <ShieldCheck size={16} className="text-accent-emerald mr-2" />
              100% Transparent Process
            </div>
            <h2 className="text-6xl md:text-[80px] font-serif text-foreground tracking-tight mb-8 leading-[0.9]">
              The Trusted Path to <br/><span className="italic text-accent-emerald relative">
                Top 0.1%
                <span className="absolute -inset-4 bg-accent-emerald/10 blur-2xl rounded-full -z-10" />
              </span>
            </h2>
            <p className="text-foreground/60 font-medium text-2xl max-w-3xl mx-auto">
        ...
        <div className="relative max-w-3xl mx-auto">
          ...
          <div className="absolute left-1/2 top-0 bottom-0 w-2 bg-black/5 -translate-x-1/2 rounded-full overflow-hidden">
            <motion.div 
          ...
          {/* The Smooth Traveling Person Emoji */}
          <motion.div 
            className="absolute left-1/2 w-20 h-20 -translate-x-1/2 -ml-1 rounded-full bg-white border border-border shadow-soft z-30 flex items-center justify-center overflow-visible"
            style={{ 
              top: glowPosition,
              scale: emojiScale
            }}
          >
            <span className="text-4xl drop-shadow-md relative z-10">
              🧑‍🚀
            </span>
            <div className="absolute inset-0 rounded-full bg-accent-emerald opacity-10 animate-pulse" />
          </motion.div>

          {/* Roadmap Steps */}
          <div className="space-y-48 relative z-10 pb-32 pt-16">
            {steps.map((step, index) => (
              <StepCard key={step.id} step={step} index={index} scrollYProgress={smoothProgress} />
            ))}
          </div>
        </div>
      </div>
