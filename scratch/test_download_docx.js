...
const getModeOrLocationForEdu = (raw, fallback) => {
  return raw || fallback;
};
...
const assertions = [
  { name: "Name Font Size Floor = 18px", check: docHtml.includes(`font-size: ${nameFontSize}px`) },
  { name: "Headline Section Titles Floor = 12px", check: docHtml.includes(`.section-title {\n        font-size: ${headlineFontSize}px;`) },
  { name: "Role Heading Floor = 11px", check: docHtml.includes(`font-size: ${subHeadlineFontSize}px`) },
  { name: "Body Summary Floor = 10px", check: docHtml.includes(`p.summary-text {\n        font-size: ${bodyFontSize}px;`) },
  { name: "Bullet Item Floor = 10px", check: docHtml.includes(`li.bullet-item {\n        font-size: ${bodyFontSize}px;`) },
  { name: "Skills Category Floor = 10px", check: docHtml.includes(`.skills-category {\n        font-size: ${bodyFontSize}px;`) },
  { name: "Education Right-Side Date = 10px", check: docHtml.includes(`style="text-align: right; font-weight: bold; font-size: ${bodyFontSize}px`) },
  { name: "Education Right-Side Location = 9px", check: docHtml.includes(`style="text-align: right; font-size: ${bodyFontSize - 1}px`) }
];
...
if (allPassed) {
  console.log("\n\x1b[32m🎉 EXCELLENT! ALL WORD DOCUMENT TYPOGRAPHY TESTS SUCCESSFULLY PASSED!\x1b[0m\n");
} else {
