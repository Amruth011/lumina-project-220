...
(async () => {
  ...
  try {
    ...
    await page.waitForSelector('button.bg-lumina-teal', { timeout: 20000 });
    ...
    console.log("5. Clicking Open Detailed Synthesis Options...");
    const clickedOptions = await page.evaluate(() => {
      const btn = Array.from(document.querySelectorAll('button')).find(b => b.textContent.includes('Open Detailed Synthesis Options'));
      if (btn) {
        btn.click();
        return "Clicked Open Options";
      }
      return "Options button not found";
    });
    console.log("Options status:", clickedOptions);
    await new Promise(r => setTimeout(r, 3000));
    ...
    const clickedGenerate = await page.evaluate(() => {
      ...
      if (btn) {
        btn.click();
        return "Clicked Generate Blueprint button";
      }
      return "Generate Blueprint button not found";
    });
    console.log("Generate status:", clickedGenerate);

    console.log("Waiting for Resume Generation (38 seconds)...");
    await new Promise(r => setTimeout(r, 38000));

    console.log("7. Scrolling resume preview into view...");
    const scrolled = await page.evaluate(() => {
      ...
      return "Scrolled to bottom fallback";
    });
    console.log("Scroll status:", scrolled);
    await new Promise(r => setTimeout(r, 3000));

    console.log("Taking final screenshot of the tailored resume preview...");
    await page.screenshot({ path: screenshotPath, fullPage: false });
    
    console.log("8. Extracting HTML of preview container...");
    ...
    console.log("SUCCESS: Screenshot and HTML saved.");
  } catch (err) {
